class CalendarEvent
  ...
  def self.for_duration(
    ...
    if week_count > 104
      ...
      time_difference = TimeDifference.between(date, today)
      months = time_difference.in_months.to_i

      # Calculate remaining days after complete months
      date_after_months = date > today ? today >> months : date >> months
      remaining_days = (today - date_after_months).to_i.abs
