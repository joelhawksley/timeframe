class HomeAssistantApi
  ...
  def fetch_states
    ...
    save_domain("home_assistant_api", JSON.parse(response.body))
  end
  ...
  def states_last_fetched_at
    domain_last_fetched_at("home_assistant_api")
  end

  def data
    domain_data("home_assistant_api")
  end
  ...
  def fetch_config
    ...
    return if response.code != 200
    save_domain("home_assistant_config_api", JSON.parse(response.body))
  end
  ...
  def config_last_fetched_at
    domain_last_fetched_at("home_assistant_config_api")
  end

  def config_data
    domain_data("home_assistant_config_api")
  end
  ...
  def fetch_calendars
    ...
    save_domain("home_assistant_calendar_api", out.compact)
  end
  ...
  def calendars_last_fetched_at
    domain_last_fetched_at("home_assistant_calendar_api")
  end

  def calendar_events
    @calendar_events ||= (domain_value("home_assistant_calendar_api")[:response] || []).map { CalendarEvent.new(**it.symbolize_keys!) }
  end
  ...
  def fetch_weather
    ...
    save_domain("home_assistant_weather_api", {
      entity_id: entity_id,
      hourly: hourly,
      daily: daily,
  ...
  def weather_last_fetched_at
    domain_last_fetched_at("home_assistant_weather_api")
  end

  def weather_data
    domain_data("home_assistant_weather_api")
  end
  ...
  def save_domain(domain, data)
    Rails.cache.write(storage_key(domain), {last_fetched_at: Time.now.utc, response: data}.to_json)
  end

  def domain_value(domain)
    JSON.parse(Rails.cache.read(storage_key(domain)) || "{}", symbolize_names: true)
  end
