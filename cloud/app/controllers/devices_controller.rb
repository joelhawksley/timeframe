...
class DevicesController < ApplicationController
  ...
  def display_view_object
    if @device.demo_mode_enabled?
      DemoDisplayContent.new.call(timezone: @device.location&.time_zone || "UTC")
    else
      config = TimeframeConfig.new
      weather_kit = if config.weatherkit? && @device.location
        WeatherKitApi.new(location: @device.location)
      end
      timezone = @device.location&.time_zone || "UTC"
      calendar_events = @device.account&.calendar_events&.future&.upcoming || []
      DisplayContent.new.call(weather_kit_api: weather_kit, timezone: timezone, calendar_events: calendar_events)
    end
  end
end
