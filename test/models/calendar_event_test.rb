...
class CalendarEventTest < Minitest::Test
  ...
  def test_for_duration_24m_less_day
    result = CalendarEvent.for_duration(date: Date.new(2023, 7, 11), today: Date.new(2025, 7, 11))

    assert_equal("24m", result.summary)
  end

  def test_for_duration_under_6_months
    result = CalendarEvent.for_duration(date: Date.new(2025, 8, 6), today: Date.new(2026, 2, 5))

    assert_equal("5m4w2d", result.summary)
  end
