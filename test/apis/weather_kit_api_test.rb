...
class WeatherKitApiTest < Minitest::Test
  ...
  def test_fetch
    VCR.use_cassette(:weatherkit_fetch, match_requests_on: [:method]) do
      client = Tenkit::Client.allocate
      client.stub :token, "test_token" do
        Tenkit::Client.stub :new, client do
          api = WeatherKitApi.new
          api.fetch

          assert api.healthy?
          assert api.data.key?(:currentWeather)
        end
      end
    end
  end
  ...
  def test_fetch_malformed_response
    ...
    client = Tenkit::Client.allocate
    response = Struct.new(:raw).new({"not_current_weather" => true})
    client.stub :weather, response do
      Tenkit::Client.stub :new, client do
        api = WeatherKitApi.new
        api.fetch
        ...
        alerts: [
          {
            ...
            urgency: "expected",
            areaName: "Barton County",
            severity: "moderate",
            certainty: "likely",
            responses: [],
            detailsUrl: "https://WeatherKitApi.apple.com/alertDetails/index.html?ids=8022c653-95e6-5761-876a-62c74c11f720&lang=en-US&timezone=America/Chicago",
  ...
  def test_hourly_calendar_events_skips_missing_hours
    travel_to DateTime.new(2023, 8, 27, 12, 0, 0, "-0600") do
      weather = {
        forecastHourly: {
          hours: [{temperature: 20.0, conditionCode: "Clear", forecastStart: "2099-01-01T00:00:00Z", precipitationType: "clear"}]
        }
      }
  ...
  def test_wind_calendar_events_no_data
    api = WeatherKitApi.new
    api.stub :data, {} do
  ...
  def test_precip_skips_past_hours
    travel_to DateTime.new(2023, 8, 27, 12, 0, 0, "-0600") do
      weather = {
        forecastHourly: {
          hours: [
            {forecastStart: "2023-08-27T10:00:00Z", precipitationType: "rain"},
            {forecastStart: "2023-08-27T20:00:00Z", precipitationType: "rain"}
          ]
        }
      }
  ...
  def test_precip_unknown_type
    travel_to DateTime.new(2023, 8, 27, 12, 0, 0, "-0600") do
      weather = {
        forecastHourly: {
          hours: [
            {forecastStart: "2023-08-27T20:00:00Z", precipitationType: "hail"}
          ]
        }
      }
      ...
      api.stub :data, weather do
        events = api.precip_calendar_events
        assert_equal 1, events.length
        assert_nil events.first.icon
      end
    end
  end

  def test_daily_calendar_events_snow_low_amount
    weather = {
      forecastDaily: {days: [{forecastStart: "2024-01-15T07:00:00Z",
                              forecastEnd: "2024-01-16T07:00:00Z",
                              conditionCode: "Snow",
                              ...
                              temperatureMax: -10.0,
                              temperatureMin: -20.0,
                              restOfDayForecast: {forecastStart: "2024-01-15T07:00:00Z",
                                                  forecastEnd: "2024-01-15T14:00:00Z",
                                                  cloudCover: 0.5,
                                                  conditionCode: "Snow",
                                                  humidity: 0.81,
                                                  precipitationAmount: 0.01,
    ...
    api.stub :data, weather do
      ...
      refute events.first.summary.include?('"')
    end
  end
  ...
  def test_wind_skips_past_hours
    travel_to DateTime.new(2023, 8, 27, 12, 0, 0, "-0600") do
      weather = {
        forecastHourly: {
          hours: [
            {forecastStart: "2023-08-27T10:00:00Z", windGust: 50.0, windDirection: 180},
            {forecastStart: "2023-08-27T20:00:00Z", windGust: 50.0, windDirection: 180}
          ]
        }
      }
